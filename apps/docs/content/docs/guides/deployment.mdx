...
Dashboard auth (sign-in, sign-up, organization ops) is issued by **apps/api** — there is no `/api/auth` route on the dashboard project. Session cookies are shared via `AUTH_COOKIE_DOMAIN=.prood.com` on the API project.
...
| Secret | Apps |
| --- | --- |
| `BETTER_AUTH_SECRET` | API, dashboard (SSR session validation), storefront |
